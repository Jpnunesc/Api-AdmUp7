...
export class EventosService {
 private pathUrlService = environment.urlService;
  ...
  search(): Observable<any> {
    return this.http.get(this.pathUrlService + 'eventos').map(res => res.json() );
  }
  ...
  edit(idEvento: number): Observable<any> {
    return this.http.get(this.pathUrlService + 'eventos/eventos/' + idEvento, this.requestOptions).map(res => res.json());
  }
